...
void setup() {
  ...
  //???xTaskCreatePinnedToCore(ControlFARM_task, "ControlFARM_task", 4096, NULL, 1, NULL, 1);
  ...
  while (1) {
    if (e32ttl100.available() > 1) {
      ...
      if (typeStr == "FRM") {
        ...
        Serial.println(*(float *)(datasensorfarm.temperature));
        tFRM = *(float *)(datasensorfarm.temperature);
        Serial.println(*(float *)(datasensorfarm.humidity));
        hFRM = *(float *)(datasensorfarm.humidity);
...
void DisplaySensor_task(void *pvParameters) {
  while (1) {
    ...
    lv_label_set_text_fmt(ui_ValTempLVR, "%d", (int)tLVR);
...
void TimePump_task(void *pvParameters) {
  while (1) {
    ...
    if (CheckTimeSVPumpAdd == true) {
      ...
      // Kiểm tra nếu phút vượt quá 60 và điều chỉnh nếu cần
      if (MinPumpPrf_add >= 60) {
        HourPumpPrf_add += 1;  // Cộng 1 vào giờ nếu phút vượt quá 60
        MinPumpPrf_add -= 60;  // Trừ 60 từ phút để đảm bảo nó nằm trong khoảng từ 0 đến 59
      }
      // Kiểm tra nếu giờ vượt quá 24 và điều chỉnh nếu cần
      if (HourPumpPrf_add >= 24) {
        HourPumpPrf_add %= 24;  // Sử dụng phép chia lấy dư để lấy giá trị hợp lệ của giờ
      }
      CheckTimeSVPumpAdd = false;  // Đánh dấu là đã thực hiện cộng giá trị
    }
